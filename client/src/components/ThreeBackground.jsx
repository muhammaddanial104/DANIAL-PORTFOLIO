// ═══════════════════════════════════════════════════
// COMPONENT: ThreeBackground.jsx — SUBTLE & DARK OVERLAY
// Reduced particle count, subtle glows, dark readability overlay
// ═══════════════════════════════════════════════════
...
export default function ThreeBackground() {
  ...
  useEffect(() => {
    ...
    // -- 1. SUBTLE BACKGROUND STARS (Reduced count & opacity) --
    const mkStars = (n, sz, col, r, op = 0.35) => {
    ...
    // Subtly distributed starfield (~2,200 points total)
    scene.add(mkStars(1200, 0.22, 0xffffff, 1000, 0.30));
    scene.add(mkStars(500, 0.30, 0xcc88ff, 800, 0.26));
    scene.add(mkStars(500, 0.26, 0x88ccff, 700, 0.24));

    // -- 2. GALAXY NEBULA (Subtle, 3,000 particles) --
    const NC = 3000;
    ...
    // -- 3. SUBTLE TORUS KNOTS --
    const knot = new THREE.Mesh(
      new THREE.TorusKnotGeometry(20, 5.5, 180, 18, 2, 3),
      new THREE.MeshBasicMaterial({ color: 0x9d00ff, wireframe: true, transparent: true, opacity: 0.05 })
    );
    ...
    const knot2 = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: 0x22d3ee, wireframe: true, transparent: true, opacity: 0.04 })
    );
    knot2.position.set(0, 0, -75);
    scene.add(knot2);

    // -- 4. SUBTLE TORUS RINGS --
    ...
    const tr1 = mkTorus(30, 0.22, 0xec4899, 0.09, Math.PI / 3, 0.2, -70);
    const tr2 = mkTorus(40, 0.16, 0x22d3ee, 0.07, -Math.PI / 4, -0.15, -70);
    const tr3 = mkTorus(20, 0.18, 0xa855f7, 0.09, Math.PI / 2, 0, -70);
    ...
    const onMouse = e => {
      ...
      tRX = -((e.clientY / window.innerHeight) - 0.5) * 0.10;
    };
    ...
    const animate = () => {
      ...
      nebula.rotation.y = t * 0.010;
      knot.rotation.x = t * 0.09;
      knot.rotation.y = t * 0.14;
      knot2.rotation.x = -t * 0.11;
      knot2.rotation.y = -t * 0.09;
      tr1.rotation.z = t * 0.05;
      ...
      floaters.forEach((f, i) => {
        ...
        f.position.y = f.userData.bY + Math.sin(t + i * 1.4) * 1.3;
      });

      camera.position.y = 14 + Math.sin(t * 0.3) * 1.0;
  ...
  return (
    <>
      ...
      />
      {/* Dark readability backdrop overlay */}
      <div
        id="bg-dark-overlay"
        style={{
          position: "fixed",
          ...
          zIndex: 0,
          pointerEvents: "none",
          background: "radial-gradient(ellipse 90% 70% at 50% 30%, rgba(3, 0, 12, 0.74) 0%, rgba(2, 0, 8, 0.91) 75%, #010005 100%)",
        }}
      />
    </>
  );
